...
from model_building_blocks import PatchEmbeddingLayer
from model_building_blocks import TransformerBlock
from model_building_blocks import create_mlp_block
...
class ViTClassifierModel(torch.nn.Module):
    ...
    def __init__(
        ...
        for _ in range(num_transformer_layers):
            self.transformer_layers.append(TransformerBlock(num_heads, embed_dim, embed_dim, feed_forward_dim))

        self.mlp_block = create_mlp_block(input_features=embed_dim, output_features=mlp_head_units, activation_function=torch.nn.GELU, dropout_rate=0.5)
